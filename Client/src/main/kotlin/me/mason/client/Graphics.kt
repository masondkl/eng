...
fun Aggregator.draw() = buffer.draw(pos, len)
...
class ShaderBuffer(val shader: Shader, size: Int = 200000, val drawBuffer: ShaderBuffer.(Int, Int) -> (Unit)) {
    ...
    fun draw(index: Int, count: Int) = drawBuffer(index, count)
    init {
...
fun Aggregator.slicedQuads(
    ...
    for (i in 0 until 9) {
        ...
        textureQuad(slicePos, sliceDim, sliceUvPos, sliceUvDim, z)
    }
}
